...
| Request-coalescing + negative-result-sentinel caching (`cachedFetchJson()`) in front of expensive reads | [`DATA_FLOW.md`](DATA_FLOW.md) Phase 2 (from the earlier full architecture review) — in-memory `Map` in front of `GET /events`, no Redis needed at this scale | PLANNED, not yet built |
