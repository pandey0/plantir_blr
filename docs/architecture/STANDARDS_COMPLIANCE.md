...
| 8 | Database Rules | ✅ mostly | Migrations ✓, indexes ✓ (`status`, `category`, `geom` GIST), pagination ✓, no N+1 found. **Known gap, tracked separately, not fixed**: `Event`→`Report`/`Evidence` has no cascade delete (see `IMPLEMENTATION_NOTES.md`'s Data model section) — would need a new migration; not urgent since nothing deletes events today. |
...
| 15 | Idempotency | ⏸ | `POST /v1/events` isn't idempotent — a retried request creates a duplicate event. No `Idempotency-Key` support. Real gap per the standards, not fixed this pass: proper support needs a dedup-key store with a TTL, which is a real feature to design, not a quick patch, and no client exists yet that actually retries. Tracked, not forgotten. |
