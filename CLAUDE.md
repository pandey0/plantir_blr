...
**Every meaningful module/component/lib directory gets its own `README.md` — not just one doc per app.** This is a standing rule, not a one-time pass: `apps/intelligence-engine/src/{events,ws,ingestion,wards}/README.md` and `apps/public-map/{lib,components/Map,components/HUD}/README.md` are the reference examples. When you add a new directory with real responsibility (not a one-off file), give it a README before moving on — don't wait to be asked, and don't let it accumulate as an undocumented gap. When you change what a module does, update its README in the same change. This applies to every repo in this ecosystem, including sibling repos outside this monorepo (e.g. `plantir-blr-data-service`) — the same discipline, not a lesser standard because it's "just a small service."
